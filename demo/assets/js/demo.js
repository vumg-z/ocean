var DEMO = {
	...
	initialize: function initialize(inIdCanvas, inParameters) {
		...
		this.ms_Camera.position.set(2398.7347918467326, 1724.3487538989368, -1801.6857102125255);

		this.ms_Camera.lookAt(new THREE.Vector3(0, 0, 0));

		// After existing initializations
		this.ms_CameraRotationRadius = Math.max(inParameters.width * 1.5, inParameters.height);
		this.ms_CameraRotationSpeed = 0.003; // Adjust for faster or slower rotation
		this.ms_CameraRotationAngle = 1; // Starting angle
	...
	update: function update() {
		...
		// Update camera rotation around the scene
		this.ms_CameraRotationAngle += this.ms_CameraRotationSpeed;
		var x = this.ms_CameraRotationRadius * Math.sin(this.ms_CameraRotationAngle);
		var z = this.ms_CameraRotationRadius * Math.cos(this.ms_CameraRotationAngle);
		this.ms_Camera.position.set(x, this.ms_Camera.position.y, z);
		this.ms_Camera.lookAt(new THREE.Vector3(0, 0, 0)); // Always look at the center

		console.log(`Camera Position: x=${this.ms_Camera.position.x}, y=${this.ms_Camera.position.y}, z=${this.ms_Camera.position.z}`);
		console.log(`Camera Rotation: x=${this.ms_Camera.rotation.x}, y=${this.ms_Camera.rotation.y}, z=${this.ms_Camera.rotation.z}`);
